...
test('control should be constructed according to generic type', () => {
  const a = new FormGroup<ControlsOfValue<NestedForm>>({ a: new FormControl(22), b: new FormControl({ a: '', c: [3] }) });
  const b = new FormGroup<ControlsOfValue<NestedForm>>({
    a: new FormControl(22),
    b: new FormGroup({ a: new FormControl('3'), c: new FormArray([new FormControl(2)]) })
  });
  const c = new FormGroup<ControlsOfValue<NestedForm>>({
    a: new FormControl(22),
    b: new FormGroup({ a: new FormControl('3'), c: new FormArray([new FormControl(33)]) }),
    c: new FormArray([new FormGroup({ a: new FormControl(3) })])
  });
  const d = new FormGroup<ControlsOfValue<NestedForm>>({
    a: new FormControl(22),
    b: new FormGroup({ a: new FormControl('3'), c: new FormArray([new FormControl(33)]) }),
...
test('control value should be of type User', () => {
  const control = new FormGroup<ControlsOfValue<User>>(null);
...
test('should be able to set validators', () => {
  const control = new FormGroup<ControlsOfValue<User>, Errors>(null);
...
test('should support nested objects', () => {
  const control = new FormGroup<ControlsOfValue<{ user: User }>>({ user: new FormControl<User>() });
